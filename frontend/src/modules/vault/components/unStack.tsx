...
export const Unstake = ({
  ...
  useEffect(() => {
    if (isUnstakeConfirmed) {
      ...
      // Invalidate backend positions
      queryClient.invalidateQueries({
        queryKey: ["backendPositions"],
      });
    }
  ...
  const isInsufficientLiquidity = amountBigInt > vaultBalance;
  ...
  return (
    <div className="w-full max-w-md relative bg-[#151320] border border-slate-800 rounded-2xl p-6 shadow-2xl overflow-hidden">
      ...
      <div className="flex flex-col gap-6">
        ...
        {isInsufficientLiquidity && selectedToken && (
          <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 text-sm">
            Insufficient vault liquidity. Available:{" "}
